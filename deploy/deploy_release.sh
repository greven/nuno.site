...
if [ $PERMISSION_ERRORS -gt 0 ]; then
  ...
  echo -e "${USER} ALL=(ALL) NOPASSWD: /usr/bin/systemctl start ${APP_NAME}"
  echo -e "${USER} ALL=(ALL) NOPASSWD: /usr/bin/systemctl stop ${APP_NAME}"
  echo -e "${USER} ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart ${APP_NAME}"
  echo -e "${USER} ALL=(ALL) NOPASSWD: /usr/bin/systemctl is-active ${APP_NAME}"
  echo -e "${USER} ALL=(ALL) NOPASSWD: /usr/bin/systemctl status ${APP_NAME}"
  echo -e ""
  echo -e "# Allow deploy user to manage ownership of application directories"
  echo -e "${USER} ALL=(ALL) NOPASSWD: /usr/bin/chown -R ${USER}\\:${USER} ${APP_DIR}/*"
  echo -e "${USER} ALL=(ALL) NOPASSWD: /usr/bin/chown -R ${USER}\\:${USER} ${DB_DIR}/*"
  echo -e "${USER} ALL=(ALL) NOPASSWD: /usr/bin/chown -R ${USER}\\:${USER} ${LOG_DIR}/*"
  echo -e "SUDOERS_EOF"
  echo ""
