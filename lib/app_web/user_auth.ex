defmodule AppWeb.UserAuth do
  ...
  defp maybe_halt(_any, %Plug.Conn{} = conn) do
    conn
    |> put_flash(:error, dgettext("authentication", "Unauthorized"))
    |> maybe_store_return_to()
    |> redirect(to: signed_in_path(conn))
    |> halt()
  end

  defp maybe_halt(true, socket), do: {:cont, socket}

  defp maybe_halt(_any, socket) do
    {:halt,
     socket
     |> LiveView.put_flash(:error, dgettext("authentication", "Unauthorized"))
     |> LiveView.push_navigate(to: signed_in_path(socket))}
  end
