...
export const TableOfContents = {
  ...
  handleOutsideClick(event) {
    // Check if click is outside the TOC container (Mobile)
    if (this.isMobile() && !this.container.contains(event.target)) {
      this.hideToc()
    }

    // Check if click is outside the TOC container and navigator (Desktop)
    if (
      !this.isMobile() &&
      !this.container.contains(event.target) &&
      !this.navigator.contains(event.target)
    ) {
      this.hideToc()
    }
  },
